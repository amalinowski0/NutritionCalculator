...
state = {
 ...
 onSubmit = async(e) => {
    ...
    if(this.state.query !=='') {
        ...
        this.setState({queryArray: ingrArray, submitted: true}, () => this.getData())
 ...
 ingredientList = () => {
    ...
    let ingredients = []
    responseData.ingredients.forEach(item => {
        ingredients.push(<div key={item.text} className="nut-ingredient-list-content">{item.text}</div>)
    })
