...
// IN THE HEAD SECTION FOR BADGE
// <script src="https://developer.edamam.com/attribution/badge.js"></script> 
// BADGE ITSELF / footer prolly
...
NutTable = () => {
    let nutArray = this.state.responseData.totalNutrients
    let table = []
    ...
    for(let i = 0;i<nutArray.length;i++)
    {
        table.push(<div className={`nut-nutTable-${nutArray[i].label}-row`}>
            <div className={`nut-nutTable-${nutArray[i].label}`}>
                {nutArray[i].label}</div>
            <div className={`nut-nutTable-quantity`}>
                {`${nutArray[i].quantity} ${nutArray[i].unit}`}
            </div>
        </div>)//moze jeszcze jeden div tutaj na daily ^^^^
    }

    return (<div className="nut-nutTable">{table}</div>)
}
    ...
    <div className="nutrition">
        <div className="nut-form">
            ...
            <form className="search-form" onSubmit={this.onSubmit}>
                <label htmlFor="analyze">Ingriedients list</label>
                <textarea className="text-ingr" placeholder="Example recipe: &#10;1 egg&#10;100g rice&#10;2 paprikas"
